const cacheName = "vector-tank-zone-pwa-20260503";
...
self.addEventListener("activate", (event) => {
  event.waitUntil(
    caches
      .keys()
      .then((keys) => Promise.all(keys.filter((key) => key.startsWith("vector-tank-zone-pwa-") && key !== cacheName).map((key) => caches.delete(key))))
