...
export class SceneBuilder {
  ...
  createPlayerMesh(): THREE.Group {
    ...
    const turretRing = new THREE.Mesh(new THREE.TorusGeometry(0.76, 0.045, 8, 32), this.materials.player);
    turretRing.rotation.x = Math.PI / 2;
    turretRing.position.y = -0.28;
    turretPivot.add(turretRing);
    const barrel = new THREE.Mesh(new THREE.BoxGeometry(0.2, 0.18, 2.4), this.materials.player);
    barrel.position.set(0, 0.05, 1.35);
